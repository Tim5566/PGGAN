...
class PGGAN():
    def __init__(self, G, D, data, noise, opts):
        ...
        self.logger = Logger('home/2022student01/Desktop/PyTorch-progressive_growing_of_gans-master/logs/' + self.current_time + "/")
        gpu = self.opts['gpu']
        ...
        self.opts['figure_dir'] = os.path.join(os.path.join(self.opts['exp_dir'], current_time), 'figure')
        os.makedirs(self.opts['sample_dir'])
        os.makedirs(self.opts['ckpt_dir'])
        os.makedirs(self.opts['figure_dir'])
    ...
    def add_noise(self, x):
        ...
        if hasattr(self, '_d_'):
            self._d_ = self._d_ * 0.9 + torch.mean(self.d_real).item() * 0.1
        else:
            self._d_ = 0.0
    ...
    def train(self):
        ...
        for R in range(from_level-1, to_level-1):
            ...
            #G,D loss counts store
            g_store_loss = []
            d_store_loss = []

            batch_size = self.bs_map[2 ** (R+1)]
            train_kimg = int(self.opts['train_kimg'] * 1000) #1000
            transition_kimg = int(self.opts['transition_kimg'] * 1000) #1000
            if R == to_level-1:
                transition_kimg = 0
            cur_nimg = 0
            ...
            for it in range(_num_it):
                ...
                # G,D loss counts store append
                g_store_loss.append(self.g_loss)
                d_store_loss.append(self.d_loss)
               
        # G,D loss line chart
            g_epochs = np.arange(len(g_store_loss))  
            d_epochs = np.arange(len(d_store_loss))
            plt.xlabel('epochs')
            plt.ylabel('loss')
            plt.plot(g_epochs, g_store_loss, color='red')
            plt.plot(d_epochs, d_store_loss, color='blue')
            plt.legend(('G','D'), loc='upper right')
            plt.savefig(os.path.join(self.opts['figure_dir'], '%dx%d loss' % (cur_resol, cur_resol)))
            plt.clf()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_kimg', default=400, type=float, help='# * 1000 real samples for each stabilizing training phase.') 
    parser.add_argument('--transition_kimg', default=400, type=float, help='# * 1000 real samples for each fading in phase.') 
    ...
    parser.add_argument('--exp_dir', default='no_tanh/exp', type=str, help='experiment dir.')
    ...
    latent_size = 512
    sigmoid_at_end = args.gan in ['lsgan', 'gan'] #true
